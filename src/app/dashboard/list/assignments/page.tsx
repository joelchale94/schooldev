  ...
  },
  {
    ...
    cell: (row: any) => (
      <div className="flex gap-2">
        <Link href={`/dashboard/list/assignments/${row.id}`}>
...
const AssignmentsListPage = () => {
  ...
  const filteredData = useMemo(() => {
    return assignmentsData.filter((assignments) => {
      const matchesSearch =
        assignments.subject.toLowerCase().includes(search.toLowerCase()) ||
        assignments.dueDate.toLowerCase().includes(search.toLowerCase()) ||
        assignments.teacher.toLowerCase().includes(search.toLowerCase()) ||
        assignments.class.toLowerCase().includes(search.toLowerCase());
