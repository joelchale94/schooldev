  ...
  },
  {
    ...
    cell: (row: any) => (
      <div className="flex gap-2">
        <Link href={`/dashboard/list/exams/${row.id}`}>
...
const ExamsListPage = () => {
  ...
  const filteredData = useMemo(() => {
    return examsData.filter((exams) => {
      const matchesSearch =
        exams.subject.toLowerCase().includes(search.toLowerCase()) ||
        exams.date.toLowerCase().includes(search.toLowerCase()) ||
        exams.teacher.toLowerCase().includes(search.toLowerCase()) ||
        exams.class.toLowerCase().includes(search.toLowerCase());
