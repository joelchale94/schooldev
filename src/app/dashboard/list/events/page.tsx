  ...
  },
  {
    ...
    cell: (row: any) => (
      <div className="flex gap-2">
        <Link href={`/dashboard/list/events/${row.id}`}>
...
const EventsListPage = () => {
  ...
  const filteredData = useMemo(() => {
    return eventsData.filter((events) => {
      const matchesSearch =
        events.title.toLowerCase().includes(search.toLowerCase()) ||
        events.date.toLowerCase().includes(search.toLowerCase()) ||
        events.class.toLowerCase().includes(search.toLowerCase());
